    ...
    systemContract
        ...
        .then((state) => {
            if (state) {
                ...
                rex.writePoint(
                    new Point('total_lent').intField('total_lent', state.total_lent.units)
                )
                rex.writePoint(
                    new Point('total_unlent').intField('total_unlent', state.total_unlent.units)
                )
                rex.writePoint(
                    new Point('total_rent').intField('total_rent', state.total_rent.units)
                )
                rex.writePoint(
                    new Point('total_lendable').intField(
                        'total_lendable',
                        state.total_lendable.units
                    )
                )
                rex.writePoint(new Point('total_rex').intField('total_rex', state.total_rex.units))
                rex.writePoint(
                    new Point('namebid_proceeds').intField(
                        'namebid_proceeds',
                        state.namebid_proceeds.units
                    )
                )
